...
s_l = function(params, const) {
  
  rho = params$rho
  sigmay = params$sigmay
  
  etay = transform_etay(params$eta, const$idx, const$Tx)
  etay_int = etay^2   #TODO Check!!
  # TODO: Check this mu too
  mu = get_muy(params, const)
  P = get_projection_matrix(etay); Pt = t(P)
  ...
  if (rho != 0){
    ...
    for (j in set){
      ...
      CI_star = PCPt_star + diag(n)*sigmay
