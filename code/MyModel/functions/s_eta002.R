s_eta002 <- function(prm, cst, mh_delta=0.05) {
  eta <- prm$eta
  acp <- prm$acp
  ...
  for (i in 1:nrow(eta)) {
    ...
    eta_l <- eta_loglike002(i, eta, prm, cst)
    eta_star_l <- eta_loglike002(i, eta_star, prm, cst)
...
eta_loglike002 <- function(i, eta, prm, cst) {
