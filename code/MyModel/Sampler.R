...
MySampler002 <- function(data, niter=5000, nburn=2000, nthin=1,
  ...
  for (i in 1:niter) {
    ...
    prm <- s_phidelta(prm, cst)  # time elapsed for 10^2 iters: 0.244
    # TODO: parallelize this eta step
    prm <- s_eta002(prm, cst, mh_delta)  # time elapsed for 10^2 iters: ?? 
    # Y regression
    prm <- s_gamma_beta(prm, cst)  # time elapsed for 10^2 iters: 46.061 *
    prm <- s_sigmay(prm, cst)  # time elapsed for 10^2 iters: 0.780
...
MySampler003 <- function(data, niter=5000, nburn=2000, nthin=1,
  ...
  for (i in 1:niter) {
    ...
    prm <- s_psi003(prm, cst)  # time elapsed for 10^2 iters: 26.589 *
    prm <- s_theta(prm, cst)  # time elapsed for 10^2 iters: 0.449 
    prm <- s_sigmax(prm, cst)  # time elapsed for 10^2 iters: 5.878
    prm <- s_phidelta(prm, cst)  # time elapsed for 10^2 iters: 0.244
    # TODO: parallelize this eta step
...
MySamplerNOGAMMA <- function(data, niter=5000, nburn=2000, nthin=1,
                                             verbose=T) {
  initials <- init_params(data$X, data$Y, 
                          data$ty, data$idy,
                          data$tx, data$idx,
